...
export const DocumentUploadProgress: React.FC<DocumentUploadProgressProps> = ({
  ...
  const startUploads = async (
    ...
    try {
      ...
      const successfulUploads = finalStatuses.filter(
        (status) => status.status === "success"
      );
      const failedUploads = finalStatuses.filter(
  ...
  const successCount = uploadStatuses.filter(
    (s) => s.status === "success"
  ).length;
          ...
          <div>
            ...
            {isComplete && (
              <p className="text-sm text-muted-foreground">
                {successCount} of {uploadStatuses.length} files uploaded
                successfully
